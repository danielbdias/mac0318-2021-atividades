...
class EnumerativeMDP:
    ...
    def transition_matrix(self, action):
        action_index = self._indexed_actions[action]
        return self.transition_matrices[action_index]

    # optimized way to compute quality function using numpy
    ...
    def describe(self):
        return pd.DataFrame(
            { 
                ...
                "Estado inicial": [','.join(self.initial_states)], 
                'Meta': [','.join(self.goal_states)] 
            }
        ).T